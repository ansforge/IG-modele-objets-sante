...
* Se placer dans le dossier [script](./script).
...
* Exécuter le script python [fsh_files_organization.py](./script/fsh_files_organization.py) qui organise les fichiers fsh par partie du MOS : `python fsh_files_organization.py`.
...
* Se place dans le dossier [script](./) et exécuter le script python [generate_plantuml.py](./script/generate_plantuml.py) qui génère les diagrammes par partie au format plantuml : `python generate_plantuml.py`.
